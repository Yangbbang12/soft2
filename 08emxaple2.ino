...
void loop() {
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      distance = _DIST_MAX + 1.0;    // Set Higher Value
      analogWrite(PIN_LED, 0);       // LED OFF
  } else if (distance < _DIST_MIN) {
      distance = _DIST_MIN - 1.0;    // Set Lower Value
      analogWrite(PIN_LED, 0);       // LED OFF
  } else {    // In desired Range
    if (distance >= 200) {
      analogWrite(PIN_LED, (300 - distance) * 2.55);
    } else {
      analogWrite(PIN_LED, (distance - 100) * 2.55);
    }
  }
